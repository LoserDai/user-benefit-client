...
import { packageApi, userApi } from '@/api/user'
...
const addToCart = async (packageItem: any) => {
  ...
  try {
    ...
    const cartData = {
      cartItems: [
        {
          ...
          itemImage: packageItem.packageImagePath || '',
          itemName: packageItem.packageName || packageItem.remark || '权益包',
          itemType: 2, // 2表示权益包
          pointPrice: packageItem.price || 0,
          quantity: 1,
          updateTime: now,
      ...
      id: 0,
      status: 1,
      totalSelectedPoints: packageItem.price || 0,
      updateTime: now,
      userId: currentUser.value.id,
    }
